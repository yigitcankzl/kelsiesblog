/**
 * Pure Three.js globe rendering logic — no React dependency.
 * Used by GlobeIntro component as a React wrapper.
 */
...
// GeoJSON ADMIN name → countryBounds name
...
/** Load CARTO dark_all @2x tiles at zoom 2 into a canvas. */
export async function loadCartoTiles(): Promise<HTMLCanvasElement> {
    const zoom = 2, grid = 4, ts = 512;
 ...
 * 1. CARTO tiles (already Mercator) — draw directly
 * 2. GeoJSON overlay with Mercator Y — same colours as Leaflet
 */
export function drawMapContent(
...
/**
 * Run the globe → flat-map animation inside a container.
 * Returns cleanup function and texture reference (for async tile update).
 */
export function runGlobeAnimation(
    ...
    // Remap UV from equirectangular to Web-Mercator
    ...
    const widthRatio  = leafletW / container.clientWidth;
    ...
    function animate(time: number) {
        ...
        if (elapsed >= SPIN && elapsed < SPIN + MORPH) {
            ...
            const rotDecay = Math.pow(1 - t, 1.5);
